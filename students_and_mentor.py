...
def Mega_Average_value(people,course):
    ...
    for i in people:
        if course in i.grades.keys():
            all_sum = all_sum + sum(i.grades[course])
            all_count = all_count  + len(i.grades[course])
        else:
            if all_sum==0:  return 0
            else:   return all_sum / all_count
...
#_/*Чистим словарь с отценками перед - Полевми испытаниями________________
...
print(f"Student1 ({Student1.Average_value()}) > Student2 "
      f"({Student2.Average_value()}) = ",
      Student1.Average_value() > Student2.Average_value())
